...
async def update_case(
    ...
    # Use exclude_unset=True (not exclude_none) so clients can explicitly clear
    # nullable fields like assigned_to by passing null in the request body.
...
async def link_events(
    ...
    # Batch-fetch all already-linked event IDs in a single query instead of N queries
    existing_result = await db.execute(
        select(CaseEvent.event_id).where(
            CaseEvent.case_id == case_id,
            CaseEvent.event_id.in_(payload.event_ids),
        )
    )
    already_linked = {row for row in existing_result.scalars()}
...
async def link_alerts(
    ...
    # Batch-fetch all already-linked alert IDs in a single query instead of N queries
    existing_result = await db.execute(
        select(CaseAlert.alert_id).where(
            CaseAlert.case_id == case_id,
            CaseAlert.alert_id.in_(payload.alert_ids),
        )
    )
    already_linked = {row for row in existing_result.scalars()}
    ...
    for alert_id in payload.alert_ids:
        if alert_id not in already_linked:
            db.add(CaseAlert(
                id=str(uuid.uuid4()),
                case_id=case_id,
                alert_id=alert_id,
