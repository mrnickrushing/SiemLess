...
const AlertDetailPanel: React.FC<{ alertId: string; onClose: () => void }> = ({ alertId, onClose }) => {
  ...
  const formatTs = (ts: string | null) => {
    if (!ts) return '—';
    try { return format(parseISO(ts), 'yyyy-MM-dd HH:mm:ss'); } catch { return ts; }
  };
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {alert.rule_name && (
                    <span className="text-xs text-cyber-muted bg-cyber-border/40 px-2 py-0.5 rounded font-mono">{alert.rule_name}</span>
                  )}
                </div>
              </div>
              {alert.description && (
                <p className="text-sm text-cyber-muted bg-cyber-bg/50 border border-cyber-border/50 rounded-lg p-3">{alert.description}</p>
              )}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-cyber-bg/50 border border-cyber-border/50 rounded-lg p-3">
                  <div className="flex items-center gap-1.5 text-xs text-cyber-muted mb-1.5"><Clock className="w-3.5 h-3.5" />Created</div>
              ...
              {(alert.mitre_tactic || alert.mitre_technique) && (
                <div className="bg-cyber-bg/50 border border-cyber-border/50 rounded-lg p-3">
                  <div className="flex items-center gap-1.5 text-xs text-cyber-muted mb-2"><Shield className="w-3.5 h-3.5" />MITRE ATT&CK</div>
                  {alert.mitre_tactic && <div className="flex gap-2 text-xs"><span className="text-cyber-muted">Tactic:</span><span className="text-cyber-text font-mono">{alert.mitre_tactic}</span></div>}
                  {alert.mitre_technique && <div className="flex gap-2 text-xs mt-1"><span className="text-cyber-muted">Technique:</span><span className="text-cyber-text font-mono">{alert.mitre_technique}</span></div>}
                </div>
              )}
              <div className="border-t border-cyber-border pt-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select value={editStatus} onChange={(e) => setEditStatus(e.target.value as AlertStatus)} className="cyber-select w-full">
                      {STATUSES.map((s) => <option key={s} value={s}>{s.replace('_', ' ').replace(/\b\w/g, (c) => c.toUpperCase())}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-cyber-muted block mb-1">Assign To</label>
                    <input type="text" value={assignTo} onChange={(e) => setAssignTo(e.target.value)} placeholder="analyst@example.com" className="cyber-input w-full" />
                  </div>
                  <div>
                    <label className="text-xs text-cyber-muted block mb-1">Notes</label>
                    <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Add investigation notes..." className="cyber-input w-full resize-none" />
                  </div>
                  {editStatus === 'false_positive' && (
                    <div>
                      <label className="text-xs text-cyber-muted block mb-1">False Positive Reason</label>
                      <input type="text" value={fpReason} onChange={(e) => setFpReason(e.target.value)} placeholder="Why is this a false positive?" className="cyber-input w-full" />
                    </div>
                  )}
                  <div className="flex justify-end gap-2 pt-1">
                    <button onClick={onClose} className="cyber-btn-secondary">Cancel</button>
                    <button onClick={handleSave} disabled={mutation.isPending} className="cyber-btn-primary">
...
const Alerts: React.FC = () => {
  ...
  const formatTs = (ts: string) => {
    try { return format(parseISO(ts), 'MMM dd, HH:mm'); } catch { return ts; }
  };
  ...
  const severityLeftBorder = (severity: string) => {
    switch (severity) {
      ...
      default:         return 'border-l-4 border-l-cyber-border';
    }
  };
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div>
            ...
            <select
              ...
              onChange={(e) => setFilters((f) => ({ ...f, status: (e.target.value as AlertStatus) || undefined, page: 1 }))}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFilters((f) => ({ ...f, severity: (e.target.value as Severity) || undefined, page: 1 }))}
              className="cyber-select text-sm"
            >
          ...
          {(filters.status || filters.severity) && (
            <button onClick={() => setFilters({ page: 1, page_size: 20 })} className="mt-4 text-xs text-cyber-muted hover:text-cyber-text flex items-center gap-1">
      ...
      {isLoading ? (
        <div className="cyber-card"><TableSkeleton rows={8} cols={5} /></div>
      ) : isError ? (
        ...
        <div className="space-y-3">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    ...
                    {alert.mitre_tactic && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-purple-900/30 text-purple-400 border border-purple-700/30 rounded font-mono">{alert.mitre_tactic}</span>
                    )}
                    {alert.mitre_technique && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-purple-900/20 text-purple-400/80 border border-purple-700/20 rounded font-mono">{alert.mitre_technique}</span>
                    )}
                  </div>
                  <h3 className="text-sm font-semibold text-cyber-text mb-1">{alert.title}</h3>
                  {alert.description && <p className="text-xs text-cyber-muted line-clamp-1">{alert.description}</p>}
                </div>
          ...
          {data && (
            <div className="cyber-card overflow-hidden">
              <Pagination page={data.page} pages={data.pages} total={data.total} pageSize={data.page_size} onPageChange={(p) => setFilters((f) => ({ ...f, page: p }))} />
            </div>
          )}
        </div>
      )}

      {selectedAlertId && <AlertDetailPanel alertId={selectedAlertId} onClose={() => setSelectedAlertId(null)} />}
    </div>
  );
};
